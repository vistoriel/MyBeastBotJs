...
export class SpawnWidget {
  ...
  async tameCallback(@Ctx() ctx: Context) {
    ...
    if (chance.bool({likelihood: beast.tameChance * 100})) {
      ...
      await ctx.editMessageCaption(await renderView('spawn', 'success', { user: ctx.callbackQuery.from }), { parse_mode: 'HTML' })
    }
    else {
      await ctx.editMessageCaption(await renderView('spawn', 'failed', { user: ctx.callbackQuery.from }), { parse_mode: 'HTML' })
    }
  }
