...
export class BeastModel extends Model {
  ...
  get level(): number {
    if (this.experience) return calcBeastLevel(this.experience);
    else return -1;
  }

  get nextLevel(): number {
    if (this.experience) return calcBeastLevel(this.experience) + 1;
    else return -1;
  }

  get levelExperience(): number {
    if (this.experience) return calcBeastExperience(this.level);
    else return -1;
  }

  get nextLevelExperience(): number {
    if (this.experience) return calcBeastExperience(this.nextLevel);
    else return -1;
  }

  get mappedNextLevelExperience(): number {
    if (this.experience) return this.nextLevelExperience - this.levelExperience;
    else return -1;
  }

  get mappedExperience(): number {
    if (this.experience) return this.experience - this.levelExperience;
    else return -1;
  }

  get health(): number {
    if (this.basicHealth && this.experience) return this.basicHealth + this.basicHealth * 0.4 * this.level;
    else return -1;
  }

  get damage(): number {
    if (this.basicDamage && this.experience) return this.basicDamage + this.basicDamage * 0.2 * this.level;
    else return -1;
  }
